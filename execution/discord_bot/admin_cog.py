...
class AdminCog(commands.Cog):
    ...
    @app_commands.command(name="bot-status", description="[Admin] Show bot statistics")
    async def status(self, interaction: discord.Interaction):
        ...
        embed.add_field(name="Total Tickets", value=str(stats["total_tickets"]), inline=True)
    ...
    @app_commands.command(name="faq-add", description="[Admin] Add a FAQ entry")
    ...
    async def faq_list(self, interaction: discord.Interaction):
        ...
        for e in entries[:15]:
            ...
            safe_q = discord.utils.escape_markdown(discord.utils.escape_mentions(e['question'][:60]))
            lines.append(f"`#{e['id']}` {votes} | **Q:** {safe_q}")

        self.db.log_audit(str(interaction.user.id), "faq_list_viewed", f"count={len(entries)}")
    ...
    async def faq_pending(self, interaction: discord.Interaction):
        ...
        for e in entries[:15]:
            safe_q = discord.utils.escape_markdown(discord.utils.escape_mentions(e['question'][:80]))
            lines.append(f"`#{e['id']}` **Q:** {safe_q}")

        self.db.log_audit(str(interaction.user.id), "faq_pending_viewed", f"count={len(entries)}")
        await interaction.response.send_message("\n".join(lines), ephemeral=True)
    ...
    async def faq_approve(self, interaction: discord.Interaction, entry_id: int,
        ...
        if answer:
            # Update the answer before approving
    ...
    @app_commands.command(name="analytics", description="[Admin] Show Nova usage analytics")
    async def analytics(self, interaction: discord.Interaction):
        ...
        lines = ["**Nova Analytics**\n"]

        # Interaction stats from learning engine
        try:
            from .learning_engine import StudentLearningEngine
            le = StudentLearningEngine()
            stats = le.get_stats()
            lines.append(f"**Usage:**")
            lines.append(f"- Total interactions: {stats.get('total_interactions', 0)}")
            ...
            lines.append("Learning engine not available.")
        ...
        self.db.log_audit(str(interaction.user.id), "analytics_viewed", "")
        await interaction.followup.send("\n".join(lines), ephemeral=True)

    @app_commands.command(name="top-questions", description="[Admin] Show most-asked questions")
    ...
    async def top_questions(self, interaction: discord.Interaction, days: int = 30):
        ...
        for i, c in enumerate(clusters, 1):
            safe_q = discord.utils.escape_markdown(discord.utils.escape_mentions(c['representative_question'][:70]))
            lines.append(
                f"{i}. **{c['frequency']}x** | `{c['category']}` | {safe_q}"
            )

        self.db.log_audit(str(interaction.user.id), "top_questions_viewed", f"days={days}")
        await interaction.response.send_message("\n".join(lines), ephemeral=True)

    @app_commands.command(name="bot-insights", description="[Admin] Analytics dashboard")
    async def insights(self, interaction: discord.Interaction):
        ...
        if clusters:
            top_q = "\n".join(
                f"**{c['frequency']}x** {discord.utils.escape_markdown(c['representative_question'][:50])}"
                for c in clusters[:5]
            )
            embed.add_field(name="Top Questions (30d)", value=top_q, inline=False)

        # Estimated API cost
        cost_in = stats["total_tokens_in"] * 0.25 / 1_000_000  # Haiku pricing
        cost_out = stats["total_tokens_out"] * 1.25 / 1_000_000
        total_cost = cost_in + cost_out
        embed.add_field(name="Est. API Cost", value=f"~${total_cost:.2f}", inline=True)
        embed.add_field(name="Injection Attempts", value=str(stats["injection_attempts"]), inline=True)

        self.db.log_audit(str(interaction.user.id), "insights_viewed", "")
        await interaction.response.send_message(embed=embed, ephemeral=True)
