#!/bin/bash
# training_officer_watch.sh — Auto-trigger Training Officer scan on file changes
# Uses fswatch to monitor directories and run scan when files change.
#
# Usage:
#   ./execution/training_officer_watch.sh          # Run in foreground
#   ./execution/training_officer_watch.sh &         # Run in background
#
...
LOG_FILE="$PROJECT_ROOT/.tmp/training-officer/watch.log"
LOCK_FILE="$PROJECT_ROOT/.tmp/training-officer/scan.lock"
COOLDOWN=60  # Minimum seconds between scans
...
if ! command -v fswatch &> /dev/null; then
    echo "[training-officer-watch] ERROR: fswatch not installed. Run: brew install fswatch"
    exit 1
fi
...
run_scan() {
    ...
    touch "$LOCK_FILE"
    log "File change detected. Running Training Officer scan..."
    ...
    python3 execution/training_officer_scan.py >> "$LOG_FILE" 2>&1
    EXIT_CODE=$?
...
log "Starting Training Officer file watcher..."
