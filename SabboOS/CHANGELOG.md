...
| Date | File | Change |
|---|---|---|
| 2026-04-03 | `directives/security-guardrails-sop.md` + `directives/browser-automation-sop.md` + `.claude/CLAUDE.md` | **NICK SARAEV ADVANCED COURSE INTEGRATION**: Analyzed 3-hour Claude Code Advanced Course (YouTube: UPtmKh1vMN8). ~80% already built from previous courses (consensus, chatroom, auto-research, pipelines, learned rules, model routing). NEW: (1) Updated security-guardrails-sop.md with 5 Claude Code-specific vulnerabilities (API key chat history exposure, supply chain/package hallucination, database RLS, public agent endpoints, credit card data) + security audit template for fresh-agent review. (2) Created browser-automation-sop.md — 3-tier decision tree (HTTP requests → Browser automation → Computer use) with scaling workflow (prototype with Playwright → reverse-engineer API → scale with HTTP → fallback to Browser Use Platform for stealth). (3) Added Built-in Claude Code Commands section to CLAUDE.md documenting `/init`, `/insights`, `/context`, and Agent Teams as native capabilities. Course reference saved to memory. |
| 2026-03-22 | `execution/catalog_pipeline.py` + `velocity_analyzer.py` + `batch_keepa_analyzer.py` + `catalog_scraper.py` + `catalog_queue.py` + `source.py` + `storefront_stalker.py` + `storefront_monitor.py` + `app.py` + `calculate_fba_profitability.py` + NEW: `gating_checker.py` + `clearance_scanner.py` + `daily_lead_scanner.py` + `reorder_tracker.py` + `deal_explainer.py` + `filter_presets.py` | **MEGA SOURCING OVERHAUL — 18-step pipeline fix + unified sourcing + smart filters + automation.** Part 1 (7 bug fixes): Fixed silent profitability calc bug (was always falling to crude 15%+$5 fallback, now uses real FBA fees by category/weight); removed $60 max price cap (was killing 58% of ShopWSS catalog); fixed confidence scoring (missing data=50 neutral, not 0 death sentence); fixed price extraction priority (Amazon price first, not Buy Box); made buy link verification non-destructive + default off; removed ungating -10 point score penalty; added StartUpFBA-style output (tag products, don't drop them). Part 2 (unified sourcing): Normalized Storefront Stalker output to Schema B; added `storefront` as 15th mode in source.py; unified `/sourcing/api/results` endpoint; connected Storefront Monitor stage-2 completions to scan_results. Part 3 (smart filters from market research): Amazon-as-seller detection (-15 confidence); FBA seller count filter (--max-sellers/--min-sellers); monthly sales filter (--min-monthly-sales); gating checker with 163 known restricted brands (high/medium/low risk); IP complaint risk detection from Keepa seller count drops. Part 4 (new capabilities): Playwright auto-fallback for 403/JS-rendered stores; robots.txt sitemap discovery; sub-sitemap index following; clearance scanner (148 stores with clearance URLs); daily lead scanner with Discord alerts (dry-run); reorder tracker with restock alerts; seasonal pattern detection from BSR history; cashback stacking calculator; AI deal explanations (Haiku-powered + rule-based fallback); 5 filter presets (beginner/intermediate/advanced/high_ticket/fast_flips); weight-based FBA fees; price freshness detection (stale >7 days); price-drop detection in prefilter; pack size mismatch hardening. Results: ShopWSS went from 285→680 filtered, 0 MAYBE→4 MAYBE, 59 SKIP→0 SKIP. All 9 new output fields: amazon_on_listing, ip_risk, gating_risk, filter_status, weight_lbs, price_age_days, price_stale, on_sale, price_drop_pct. |
